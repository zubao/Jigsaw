package com.lipeilong.jigsaw.config;

import android.content.Context;

import com.lipeilong.jigsaw.env.Env;
import com.lipeilong.jigsaw.util.FileUtil;

import java.io.File;

public class PathConfig {

	// PS：注意程序中所有出现的路径，结尾都不带 "/"
	
	/**
	 * 获得cache的根目录
	 * @return
	 */
	public static String getCacheDir(){
		
		Context context = Env.getContext();
		
		// 先尝试选择外部缓存
		File file = context.getExternalCacheDir();
		if (file != null) {
		    String extCacheDir = file.getAbsolutePath();
		    FileUtil.ensureDir(extCacheDir);
		    if (FileUtil.isDirExist(extCacheDir)) {
		        return extCacheDir;
		    }
        }
		
		// 暂尝试选择内部缓存
		String intcacheDir = context.getCacheDir().getAbsolutePath();
		FileUtil.ensureDir(intcacheDir);
		return intcacheDir;
		
		// PS: obb用于存放应用下载回来的数据包，这里不使用
		//String obbDir = context.getObbDir().getAbsolutePath();
	}
	
	
	/**
     *  发帖时，图片、视频的输出目录
     * 
     * @return
     */
    public static final String getCameraOutputDir() {
        String dir = PathConfig.getCacheDir() + "/camera";
        FileUtil.ensureDir(dir);
        if (FileUtil.isDirExist(dir)) {
            return dir;
        }
        
        // 如果实在是创建失败则直接返回缓存根目录
        return PathConfig.getCacheDir();
    }
    

    /**
     * 发帖时，拍照输出路径（相册选择照片发送，也存这个路径）
     * @return
     */
    public static final String getCameraOutputPicPath(){
        return getCameraOutputDir() + "/pic.jpg";
    }
    
    /**
     * 发帖时，视频输出路径
     * @return
     */
    public static final String getCameraOutputVideoPath(){
        return getCameraOutputDir() + "/video.mp4";
    }

 
    


}
